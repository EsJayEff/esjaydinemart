...
async function fetchAllProductsData() {
  let res = await fetch(`https://${process.env.NEXT_PUBLIC_SANITY_PROJECT_ID}.api.sanity.io/v2023-07-04/data/query/production?query=*%5B_type+%3D%3D+%22products%22+%26%26+productTypes%5B1%5D+%3D%3D+%22Male%22%5D%0A++`, {
    next: {
      revalidate: 60
    }
  });
...
const Male = async ({ params }: { params: { mtype: string } }) => {
  ...
  if (params.mtype !== "Male"){
    let orginalSortedDataOfParams = res.result.filter((items: oneProductType) => items.productTypes[0] === params.mtype)
